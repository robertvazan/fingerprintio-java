...
public class Ansi378v2004Fingerprint {
	...
	Ansi378v2004Fingerprint(TemplateReader in, boolean lax) {
		...
		scanType = TemplateUtils.decodeType(offsetAndType & 0xf, Ansi378v2004ScanType.values(), t -> t.code, lax, "Unrecognized sensor type code.");
		quality = in.readUnsignedByte();
		int count = in.readUnsignedByte();
		for (int i = 0; i < count; ++i)
			minutiae.add(new Ansi378v2004Minutia(in, lax));
		...
		while (readBytes < totalBytes) {
			...
			if (extension.type == Ansi378v2004CountExtension.IDENTIFIER)
				decodeExtension(extension, data -> counts = new Ansi378v2004CountExtension(data, lax), lax, "Unable to decode ridge count extension.");
			else if (extension.type == Ansi378v2004CoreDeltaExtension.IDENTIFIER)
				decodeExtension(extension, data -> coredelta = new Ansi378v2004CoreDeltaExtension(data, lax), lax, "Unable to decode core/delta extension.");
			else
