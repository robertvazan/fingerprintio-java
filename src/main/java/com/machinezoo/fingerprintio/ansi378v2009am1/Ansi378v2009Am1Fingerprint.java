...
public class Ansi378v2009Am1Fingerprint {
	...
	Ansi378v2009Am1Fingerprint(DataInputBuffer in, boolean lax) {
		...
		for (int i = 0; i < count; ++i)
			minutiae.add(new Ansi378v2009Am1Minutia(in, lax));
		...
		while (readBytes < totalBytes) {
			...
			if (extension.type == Ansi378v2009Am1CountExtension.IDENTIFIER)
				decodeExtension(extension, data -> counts = new Ansi378v2009Am1CountExtension(data, lax), lax, "Unable to decode ridge count extension.");
			else if (extension.type == Ansi378v2009Am1CoreDeltaExtension.IDENTIFIER)
				decodeExtension(extension, data -> coredelta = new Ansi378v2009Am1CoreDeltaExtension(data, lax), lax, "Unable to decode core/delta extension.");
			else
