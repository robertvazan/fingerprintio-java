...
public class Ansi378CoreDeltaExtension {
	...
	public List<Ansi378Delta> deltas = new ArrayList<>();
	public Ansi378CoreDeltaExtension() {
	}
	Ansi378CoreDeltaExtension(byte[] extension, boolean lax) {
		TemplateUtils.decodeExtension(extension, in -> {
			int coreInfo = in.readUnsignedByte();
			for (int i = 0; i < (coreInfo & 0xf); ++i)
				cores.add(new Ansi378Core(in, (coreInfo & 0x40) != 0));
			int deltaInfo = in.readUnsignedByte();
			for (int i = 0; i < (deltaInfo & 0xf); ++i)
				deltas.add(new Ansi378Delta(in, (deltaInfo & 0x40) != 0));
