...
public class Ansi378TemplateTest {
	@Test public void roundtrip() {
		...
		t.vendorId = 0x5678;
		t.subformat = 0x1234;
		Ansi378Fingerprint f = new Ansi378Fingerprint();
		f.position = Ansi378Position.LEFT_RING;
		f.scanType = Ansi378ScanType.NONLIVE_ROLLED;
		f.quality = 45;
		Ansi378Minutia m = new Ansi378Minutia();
		m.type = Ansi378MinutiaType.ENDING;
		m.positionX = 500;
		m.positionY = 300;
		m.angle = 99;
		m.quality = 77;
		f.minutiae.add(m);
		m = new Ansi378Minutia();
		m.type = Ansi378MinutiaType.BIFURCATION;
		f.minutiae.add(m);
		Ansi378CountExtension cx = f.counts = new Ansi378CountExtension();
		cx.type = Ansi378CountType.OCTANTS;
		Ansi378CountEdge e = new Ansi378CountEdge();
		e.from = 0;
		e.to = 1;
		e.count = 15;
		cx.edges.add(e);
		e = new Ansi378CountEdge();
		e.from = 1;
		e.to = 0;
		e.count = 25;
		cx.edges.add(e);
		Ansi378CoreDeltaExtension cd = f.coredelta = new Ansi378CoreDeltaExtension();
		Ansi378Core c = new Ansi378Core();
		c.positionX = 123;
		c.positionY = 234;
		c.angle = 55;
		cd.cores.add(c);
		Ansi378Delta d = new Ansi378Delta();
		d.positionX = 111;
		d.positionY = 222;
		d.angles = new int[] { 10, 20, 30 };
		cd.deltas.add(d);
		Ansi378Extension x = new Ansi378Extension();
		x.type = 0x4455;
		x.data = new byte[] { 1, 2, 3 };
		f.extensions.add(x);
		t.fingerprints.add(f);
		f = new Ansi378Fingerprint();
		f.position = Ansi378Position.LEFT_LITTLE;
		t.fingerprints.add(f);
		f = new Ansi378Fingerprint();
		f.position = Ansi378Position.LEFT_RING;
		f.view = 1;
		t.fingerprints.add(f);
		t = new Ansi378Template(t.toByteArray());
		...
		Ansi378Fingerprint fa = t.fingerprints.get(0);
		Ansi378Fingerprint fb = t.fingerprints.get(1);
		Ansi378Fingerprint fc = t.fingerprints.get(2);
		assertSame(Ansi378Position.LEFT_RING, fa.position);
		assertSame(Ansi378Position.LEFT_LITTLE, fb.position);
		assertSame(Ansi378Position.LEFT_RING, fc.position);
		assertEquals(0, fa.view);
		assertEquals(0, fb.view);
		assertEquals(1, fc.view);
		assertSame(Ansi378ScanType.NONLIVE_ROLLED, fa.scanType);
		assertEquals(45, fa.quality);
		assertEquals(2, fa.minutiae.size());
		Ansi378Minutia ma0 = fa.minutiae.get(0);
		assertSame(Ansi378MinutiaType.ENDING, ma0.type);
		assertEquals(500, ma0.positionX);
		assertEquals(300, ma0.positionY);
		assertEquals(99, ma0.angle);
		assertEquals(77, ma0.quality);
		Ansi378Minutia ma1 = fa.minutiae.get(1);
		assertSame(Ansi378MinutiaType.BIFURCATION, ma1.type);
		cx = fa.counts;
		assertSame(Ansi378CountType.OCTANTS, cx.type);
		assertEquals(2, cx.edges.size());
		Ansi378CountEdge ce0 = cx.edges.get(0);
		assertEquals(0, ce0.from);
		assertEquals(1, ce0.to);
		assertEquals(15, ce0.count);
		Ansi378CountEdge ce1 = cx.edges.get(1);
	...
	@Test public void spec() {
		...
		Ansi378Fingerprint fa = t.fingerprints.get(0);
		assertSame(Ansi378Position.LEFT_INDEX, fa.position);
		assertEquals(90, fa.quality);
		assertSame(Ansi378ScanType.LIVE_PLAIN, fa.scanType);
		...
		Ansi378Minutia ma26 = fa.minutiae.get(26);
		assertSame(Ansi378MinutiaType.BIFURCATION, ma26.type);
		assertEquals(126, ma26.positionX);
		assertEquals(115, ma26.positionY);
		assertEquals(30, ma26.quality);
		Ansi378Fingerprint fb = t.fingerprints.get(1);
		assertSame(Ansi378Position.RIGHT_INDEX, fb.position);
		assertEquals(70, fb.quality);
		assertSame(Ansi378ScanType.LIVE_PLAIN, fb.scanType);
		assertEquals(22, fb.minutiae.size());
		Ansi378Minutia mb0 = fb.minutiae.get(0);
		assertSame(Ansi378MinutiaType.ENDING, mb0.type);
		assertEquals(40, mb0.positionX);
		assertEquals(93, mb0.positionY);
		assertEquals(90, mb0.quality);
		Ansi378Minutia mb21 = fb.minutiae.get(21);
		assertSame(Ansi378MinutiaType.BIFURCATION, mb21.type);
	...
	private Ansi378Template decode() {
		byte[] bytes = sample();
		/*
		 ...
		 */
		bytes[4] = ' ';
		assertTrue(Ansi378Template.accepts(bytes));
		/*
		 * Sample template in the spec is bogus. Check for exception. Then enable permissive decoding.
		 */
		try {
			new Ansi378Template(bytes);
			fail();
		} catch (Throwable ex) {
			assertTrue(ex instanceof TemplateFormatException);
		}
		return new Ansi378Template(bytes, true);
	}
	static byte[] sample() {
		/*
		 * Sample template from the spec.
		 */
		return TestUtils.resource(Ansi378TemplateTest.class, "ansi378-sample.dat");
	}
}
