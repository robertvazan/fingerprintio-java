...
public class Ansi378v2009TemplateTest {
	@Test public void roundtrip() {
		...
		Ansi378CountEdge ce = new Ansi378CountEdge();
		ce.from = 0;
		ce.to = 1;
		ce.count = 15;
		cx.edges.add(ce);
		ce = new Ansi378CountEdge();
		ce.from = 1;
		ce.to = 0;
		ce.count = 25;
		cx.edges.add(ce);
		...
		assertSame(Ansi378Position.LEFT_RING, fb.position);
		assertSame(Ansi378Position.LEFT_LITTLE, fc.position);
	...
	@Test public void spec() {
		...
		Ansi378CountExtension cx = fb.counts;
		assertSame(Ansi378CountType.QUADRANTS, cx.type);
		assertEquals(2, cx.edges.size());
		Ansi378CountEdge ce0 = cx.edges.get(0);
	...
	static byte[] sample() {
		 ...
		 */
		return TestUtils.resource(Ansi378TemplateTest.class, "ansi378v2009-sample.dat");
	}
}
